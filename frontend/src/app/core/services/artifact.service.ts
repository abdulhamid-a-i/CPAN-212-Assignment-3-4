import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { firstValueFrom } from 'rxjs';

@Injectable({ providedIn: 'root' })
export class ArtifactService {
  private readonly apiUrl = 'http://localhost:3000/api/artifacts';

  constructor(private http: HttpClient) {}

  async getAll(id: string): Promise<any> {
    return await firstValueFrom(
      this.http.get<any[]>(`${this.apiUrl}/${id}`, { withCredentials: true })
    );
  }

  async getById(id: string): Promise<any> {
    return await firstValueFrom(
      this.http.get(`${this.apiUrl}/edit/${id}`, { withCredentials: true })
    );
  }

  async create(data: { title: string; description: string, projectId: string }): Promise<any> {
    return await firstValueFrom(
      this.http.post(this.apiUrl, data, { withCredentials: true })
    );
  }

  async update(id: string, data: { title: string; description: string }): Promise<any> {
    return await firstValueFrom(
      this.http.put(`${this.apiUrl}/${id}`, data, { withCredentials: true })
    );
  }

  async delete(id: string): Promise<any> {
    return await firstValueFrom(
      this.http.delete(`${this.apiUrl}/${id}`, { withCredentials: true })
    );
  }
}